(function($, $$){
	...
	var col = 13;
	var row = 20;
	$main.get(0).width = dwidth * 0.7;
	var item1 = dwidth * 0.7 / col;
	var item2 = dheight / row;
	
	var minitem = Math.min(item1, item2);		// 最小item
	
	$next.get(0).width = dwidth * 0.25;
	$next.get(0).height = 100;
	$main.get(0).height = minitem * row;
	$(".right").css("width", dwidth*0.3 -4 + "px");
	$(".right").css("height", minitem * row + "px");

	var $document = $(document);	// 获取document对象，主要是为了键盘事件的调用
	var $$main = $$("#main");		// zepto获取对象.
	...
	// 生成随机样式颜色
		...
		drawNextCom : function(){
			...
			for (var i = obj.arr.length - 1; i >= 0; i -= 2) {
				$next.drawRect({
					...
					strokeWidth: 1,
					fillStyle: obj.color,
					x: this.item * obj.arr[i],
					y: this.item * obj.arr[i-1],
					width: this.item - 1,
					height: this.item - 1,
					cornerRadius: 5,
					fromCenter: false
		...
		redraw : function(){
			...
			this.clearAllDraw($main, dwidth*0.7, minitem * row);		// 清除整个画板的元素块儿
			for(var i = 0; i < row; i++){
				for(var j =0; j < col; j++){
					if(boxArr[i][j] == 1){
	...
	Controller.prototype = {
		...
		gameOver : function(){
			...
			$$main.off("tap");
			$$main.off("swipeDown");
			this.model.clearAllDraw($main,dwidth*0.7, minitem * row);
			this.model.clearAllDraw($next, dwidth*0.25, 100);		// 清除下一块儿面板元素块儿
	...
	toggle.Id = 0;
	function toggle(obj, first, second){
